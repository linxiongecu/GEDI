...
plotWFMetrics = function(level1b, level2a, shot_number, rh=c(25, 50, 75), ...) {
  ...
  print(qts)
  print(qts+c(0, 0.1*abs(diff(qts))))
  par(new=TRUE)
  ...
  #add ground line 
  abline(ground_z, 0, lty="dashed", col="red")
  #add label
  text(qts[2]-diff(qts)*4/5, ground_z, paste0("Ground: ", round(ground_z, digits = 2), "m"), pos=3)
